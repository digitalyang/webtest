...
export async function handleCreateRoleRequest(request, env) {
  ...
  try {
    ...
    if (body?.targetType === "static") {
      return json(await createStaticPortfolioRole(env, body), 201);
    }
...
export async function handleSetCoverRequest(request, env) {
  ...
  try {
    ...
    if (body?.targetType === "static-work" || body?.targetType === "static-role") {
      return json(await setStaticPortfolioCover(env, body));
    }
...
export async function createStaticPortfolioRole(env, body) {
...
export async function setStaticPortfolioCover(env, { targetType, targetId, imageId }) {
  const staticTargetType = targetType === "static-work" ? "work" : "role";
  const cleanTargetId = cleanRequiredText(targetId, "Cover target id is required");
  const numericImageId = cleanRequiredNumber(imageId, "Image id is required");
...
ON CONFLICT(target_type, target_id) DO UPDATE SET
  ...
  ).bind(staticTargetType, cleanTargetId, numericImageId).run();
}
