...
export function renderRoleImages(images = []) {
  return withoutThumbnails(images)
    .map(
      (image) => `
        <a class="portfolio-thumb portfolio-original-thumb portfolio-fit-contain" href="${resolveImageSrc(image.src)}" target="_blank" rel="noopener noreferrer">
          <img src="${resolveImageSrc(image.src)}" alt="${image.alt}" loading="lazy" width="480" height="640">
          ${image.coserName ? `<span class="portfolio-credit-badge">CN：${image.coserName}</span>` : ""}
        </a>`
    )
    .join("");
}
