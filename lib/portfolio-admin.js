...
export function applyStaticPortfolioExtensions(
  staticManifest = {},
  { staticImages = [], staticRoles = [], coverOverrides = [] } = {}
) {
  const appendedImages = staticImages.filter((image) => !isHidden(image)).sort(compareRows);
  const appendedRoles = staticRoles.filter((role) => !isHidden(role)).sort(compareRows);
  const coverImagesById = new Map(appendedImages.map((image) => [String(image.id), normalizeImage(image)]));
  ...
  return {
    ...
    photographyWorks: (staticManifest.photographyWorks || []).map((work) => {
      const existingRoles = (work.roles || []).map((role) => ({ ...role, images: [...(role.images || [])] }));
      ...
      const roles = [...existingRoles, ...newRoles].map((role) => {
        const roleImages = appendedImages
          .filter((image) => idsMatch(image.static_role_id, role.id))
          .map((image) => {
            const normalizedImage = normalizeImage(image);
            return { src: normalizedImage.src, alt: normalizedImage.alt };
          });
        const images = [...(role.images || []), ...roleImages];
        const coverOverride = roleCoverOverrides.get(String(role.id));

        return {
          ...role,
          coverThumb: coverOverride?.coverThumb || role.coverThumb || "",
      ...
      return {
        ...work,
        coverThumb: workCoverOverride?.coverThumb || work.coverThumb || "",
...
export function buildAdminPortfolioOptions({ staticManifest = {}, dynamicRows = {}, staticRoles = [] } = {}) {
  ...
  for (const work of staticWorks) {
    ...
    const workStaticRoles = visibleStaticRoles.filter((role) => idsMatch(role.static_work_id, work.id));
