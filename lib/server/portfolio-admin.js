...
export async function handleAdminPortfolioRequest(request, env) {
...
export async function handleCreateImagesRequest(request, env) {
  const unauthorized = await requireAdminOrJson(request, env);
  if (unauthorized) return unauthorized;
  ...
  try {
    ...
    if (body?.targetType === "static") {
      return handleCreateStaticImagesRequest(body, env);
    }
...
export async function handleImageUploadPlanRequest(request, env) {
  const unauthorized = await requireAdminOrJson(request, env);
  if (unauthorized) return unauthorized;
  ...
  try {
    const body = await readJson(request);
    if (body?.targetType === "static") {
      const nextIndex = await getNextStaticImageIndex(env, body?.staticRoleId, body?.staticImageCount);
      const plan = buildStaticImageUploadPlan({
        staticWorkId: body?.staticWorkId,
        staticRoleId: body?.staticRoleId,
...
async function handleCreateStaticImagesRequest(body, env) {
  const staticWorkId = cleanRequiredText(body?.staticWorkId, "Static work id is required");
  const staticRoleId = cleanRequiredText(body?.staticRoleId, "Static role id is required");
  const roleSlug = getStaticRoleSlug(staticRoleId, staticWorkId);
  const images = Array.isArray(body?.images) ? body.images : [];
...
export async function getDynamicPortfolioRows(env, { includeHidden = false } = {}) {
  ...
  const [works, roles, images] = await Promise.all([
...
export async function getMergedPortfolioData(env, staticManifest) {
  ...
  return mergePortfolioData(staticManifest, normalizeDynamicPortfolio(rows));
}
