...
export async function setPortfolioCover(env, { targetType, targetId, imageId }) {
  ...
  await ensureDynamicCoverThumb(env, imageId);
...
async function ensureDynamicCoverThumb(env, imageId) {
  const image = await env.DB.prepare(
    "SELECT id, cloudinary_public_id, cover_thumb_url FROM portfolio_images WHERE id = ? LIMIT 1"
  ).bind(imageId).first();
  ...
  ).bind(toCoverThumbUrl(env, image.cloudinary_public_id), imageId).run();
}
