...
export { COMPRESSION_TARGET_BYTES } from "../portfolio-compression-policy.js";

const WEBP_TYPE = WEBP_MIME;
...
function canvasToBlob(canvas, type, quality) {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      if (type === WEBP_TYPE && blob.type && blob.type !== WEBP_TYPE) {
        reject(new Error("当前浏览器不支持 WebP 自动压缩，请先手动压缩图片到 9.5MB 以下。"));
        return;
      }
